...
const API_BASE = "https://script.google.com/macros/s/AKfycbxB-VOeansLHzKIpQShM64w2ZbBCVb_YLEi5IBZ7MfF2r_gUTQLKzMp1nHkmwo8pmRf/exec";
...
scanQrBtn.addEventListener('click', async () => {
    if (qrReaderContainer.style.display === 'none') {
        ...
        try {
            ...
            if (cameras.length > 0) {
                // ค้นหากล้องหลัง (ถ้ามี) ถ้าไม่มีก็ใช้กล้องหน้า
                let backcamera = cameras.find(cam => cam.label.toLowerCase().includes('back') || cam.label.toLowerCase().includes('rear'))?.id || cameras[0]?.id;
